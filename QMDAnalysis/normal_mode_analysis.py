...
class FrequencyTypeError(TypeError):
    def __init__(self, msg='Requires a Frequency object', *args, **kwargs):
...
def normal_mode_matrix(vibrational_structure, mass_weight=True) -> npt.NDArray:
    ...
    defines the collective normal modes. Can be optionally mass weighted. Requires an instance of `Frequency`.

    Parameters
    ----------

    vibrational_structure : Frequency
    ...
    """
    if not isinstance(vibrational_structure, Frequency):
        raise FrequencyTypeError('An instance of Frequency is required to calculate the normal mode matrix')
...
def normal_mode_transform(molecule, ref_structure, mass_weighted=True) -> npt.NDArray:
    ...
    By default the coordinates are not mass weighted. Requires a `Molecule` and `Frequency` instance.
    ...
    if not isinstance(ref_structure, Frequency):
        raise FrequencyTypeError('An instance of Frequency is required to project the molecule onto')
...
def nma_traj(trajectory, ref_structure, time_intervals: list) -> tuple[npt.NDArray, npt.NDArray]:
    ...
    if not isinstance(ref_structure, Frequency):
        raise FrequencyTypeError('An instance of Frequency containing reference normal modes is required to project the trajectory onto')
...
def nma_ensemble(ensemble, ref_structure, time_intervals: list):
    """
    A function to perform normal mode analysis on a whole ensemble of trajectories given a reference Frequencyal
...
if __name__ == "__main__":
    ...
    ref_structure = Frequency('/Users/kyleacheson/PycharmProjects/PyQOFTA/data/Freq/cs2.molden')
